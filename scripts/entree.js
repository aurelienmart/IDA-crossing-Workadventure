...
WA.onEnterZone('Pasc',()=>{WA.disablePlayerControls();WA.openPopup('tuto','Bonjour et bienvenue sur Workadventure !',[{label:'Où suis-je ?',className:'primary',callback:(popup)=>{popup.close();WA.openPopup('tuto','Tu es dans une simulation des bureaux de Wavestone de Paris',[{label:'Suite',className:'primary',callback:(popup)=>{popup.close();WA.openPopup('tuto','Tu peux discuter avec tes collègues grace à ton micro et ta caméra ou à  travers du chat !',[{label:'Suite',className:'primary',callback:(popup)=>{popup.close();WA.openPopup('tuto','Accède à waveplace depuis les ordinateurs du 17ème étage ou bien rejoins tes amis à la machine à café !',[{label:'Suite',className:'primary',callback:(popup)=>{popup.close();WA.openPopup('tuto',"Workadventure est encore en chantier, certains étages ne sont pas encore finis mais l’équipe IDA Crossing y travaille d’arrache-pied !",[{label:'Close',className:'primary',callback:(popup)=>{popup.close();WA.restorePlayerControls()}}])}},])}},])}},])}},{label:' Je connais !',className:'primary',callback:(popup)=>{popup.close();WA.openPopup('tuto','Je te laisse alors, à bientôt !',[{label:'Close',className:'primary',callback:(popup)=>{popup.close();WA.restorePlayerControls()}}])}},]);})